...
const navIcons: Record<(typeof navPages)[number], ReactNode> = {
  explorer: (
...
export default function Nav({ activePage, onNavigate }: NavProps) {
  ...
  const navLabels: Record<(typeof navPages)[number], string> = {
    ...
    reputacion: t.nav.reputation,
    soporte: t.nav.support,
    configuracion: t.nav.settings,
  };
  ...
  return (
    <aside className="w-64 flex-shrink-0 flex flex-col border-r border-white/5 overflow-y-auto no-scrollbar bg-brand-sidebar">
      <div className="p-6">
        ...
        <p className="text-[10px] text-gray-500 uppercase tracking-widest mt-2">{t.brand.tagline}</p>
      </div>
      ...
      <div className="p-4 mt-auto">
        <div className="bg-[#1e1e1e] rounded-2xl p-4 border border-white/5">
          <div className="flex items-center space-x-2 mb-2">
            ...
            <span className="text-xs font-bold">{t.nav.safePurchase}</span>
          </div>
          <p className="text-[10px] text-gray-400 leading-relaxed mb-3">{t.nav.safePurchaseBody}</p>
          <button type="button" className="text-[10px] text-brand-purple hover:underline flex items-center">
            {t.nav.learnMore}
