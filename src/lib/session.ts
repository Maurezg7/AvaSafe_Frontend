...
export function persistSession(data: { token: string; address?: string }) {
  localStorage.setItem(TOKEN_KEY, data.token);

  const payload = decodeJwtPayload(data.token);
  const address = data.address ?? (payload?.address as string | undefined) ?? getStoredUser()?.address;

  localStorage.setItem(USER_KEY, JSON.stringify({ address } as AuthUser));
}
